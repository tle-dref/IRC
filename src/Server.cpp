...
/*   Updated: 2025/01/16 19:05:29 by gbruscan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	std::string response = "421 " + clients.getNickname(fd) + " " + cmd
		+ " :Unknown command\n";
...
void Server::handleClientMessage(const std::string &message, int fd)
{
	...
	while (std::getline(ss, line, '\n'))
	{
		...
		if (cmd.isValid())
		{
			...
			if (client == NULL)
			{
				std::cerr << "Erreur : Client avec fd " << fd << " non trouvé." << std::endl;
				continue ;
			}
				...
				&& !client->isAuthenticated)
			{
				...
				if (send(fd, errorMsg.c_str(), errorMsg.size(), MSG_NOSIGNAL) ==
					-1)
				{
					std::cerr << "Erreur : Échec de l'envoi du message d'erreur au client fd " << fd << std::endl;
				}
				continue ;
			}
